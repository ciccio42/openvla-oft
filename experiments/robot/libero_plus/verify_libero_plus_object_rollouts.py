...
DEFAULT_ROLLOUT_ROOT = Path(
    "/mnt/beegfs/frosa/Multi-Task-LFD-Framework/repo/openvla-oft/experiments/robot/libero_plus/rollouts/libero_object"
)
DEFAULT_OUTPUT_SH = Path(
    "/mnt/beegfs/frosa/Multi-Task-LFD-Framework/repo/openvla-oft/experiments/robot/libero_plus/run_libero_plus_eval_resume_incomplete.sh"
)
...
def main() -> None:
    ...
    if incomplete:
        ...
        for exp_name, count, ext, change_spawn, train_spawn_distribution, run_number in incomplete:
            ...
            lines.append(
                "srun torchrun --standalone --nnodes 1 --nproc-per-node 1 run_libero_plus_eval.py \\"
            )
