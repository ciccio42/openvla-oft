...
DATASET_NAME = "ur5e_pick_place_abs_pose"
...
def run_bash_script(first_run=False):
    ...
    for i in range(2, 20):
        ...
        print(f"Running bash script with arguments: {BASH_ARGUMENTS}")
        result = subprocess.run(['sbatch', 'finetune_abs_pose.sh'] + BASH_ARGUMENTS, capture_output=True, text=True)
        if result.returncode != 0:
